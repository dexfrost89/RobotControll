...
class PPOLightning(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        path_to_environment = "/home/dexfrost89/Загрузки/Telegram Desktop/build/RobotBalance.x86_64"
        unity_environment = UnityEnvironment(
            file_name=path_to_environment, no_graphics=True,
            seed=1, side_channels=[], base_port=6005, worker_id=1
        )
        self.env = UnityToGymWrapper(unity_environment, allow_multiple_obs=False)
        # self.env = gym.make(env)
        # value network
        # observation_space_shape = self.env.group_spec.observation_specs[1].shape
...
def main(args) -> None:
    model = PPOLightning.load_from_checkpoint("/home/dexfrost89/Загрузки/Telegram Desktop/epoch=477-step=1911_bk.ckpt")

    path_to_environment = "/home/dexfrost89/Загрузки/Telegram Desktop/build/RobotBalance.x86_64"
    unity_environment = UnityEnvironment(
        file_name=path_to_environment,
        seed=1, side_channels=[], base_port=6005, worker_id=2
    )
    env = UnityToGymWrapper(unity_environment, allow_multiple_obs=False)

    obs = torch.FloatTensor(env.reset())

    for i in range(400):
        action = model.forward(obs)
        obs, reward, done, info = env.step(action[1].numpy())
