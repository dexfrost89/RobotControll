...
class IntentionActor(torch.nn.Module):
    ...
    def init_weights(self):
        ...
        torch.nn.init.xavier_uniform(self.layer1.weight.data)
        torch.nn.init.constant(self.layer1.bias.data, 0)
        torch.nn.init.xavier_uniform(self.final_layer1.weight.data)
        torch.nn.init.constant(self.final_layer1.bias.data, 0)
        torch.nn.init.xavier_uniform(self.final_layer2.weight.data)
        torch.nn.init.constant(self.final_layer2.bias.data, 0)

    def forward(self, x):
        x = self.non_linear(self.layer1(x))
        y = self.clamp(self.final_non_linear(self.final_layer2(x)))
        x = self.final_non_linear(self.final_layer1(x))
        return x, y
...
class Actor(SQXNet):
    ...
    def forward(self, x, intention, log_prob=False):
        x, y = super().forward(x, intention)
        # Intention head determines parameters of Categorical distribution
        dist = torch.distributions.Normal(x, y)
        action = dist.sample()
        if log_prob:
